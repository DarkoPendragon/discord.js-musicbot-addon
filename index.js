  ...
  try {
    ...
    class Music {
      ...
      canSkip(member, queue) {
        ...
        else return !!this.isAdmin(member);
      };
    ...
    let musicbot = new Music(client, options);
    ...
    musicbot.playFunction = (msg, suffix) => {
      ...
      if (searchstring.startsWith('http') && searchstring.includes("list=")) {
        ...
        let playid = searchstring.toString()
            .split('list=')[1];
        if (playid.toString()
        ...
        new Promise(async (resolve) => {
          ...
          if (msg.channel.permissionsFor(msg.guild.me).has('EMBED_LINKS')) {
            ...
            try {
              ...
              let songTitle = res.title.replace(/\\/g, '\\\\')
                  .replace(/`/g, '\`')
                  .replace(/\*/g, '\\*')
                  .replace(/_/g, '\\_')
                  .replace(/~/g, '\\~');
    ...
    musicbot.helpFunction = (msg, suffix) => {
    ...
    musicbot.npFunction = (msg) => {
      ...
      if (msg.channel.permissionsFor(msg.guild.me)
        ...
        try {
          ...
          let songTitle = queue.last.title.replace(/\\/g, '\\\\')
              .replace(/`/g, '\`')
              .replace(/\*/g, '\\*')
              .replace(/_/g, '\\_')
              .replace(/~/g, '\\~');
    ...
    musicbot.queueFunction = (msg, suffix) => {
      ...
      if (suffix) {
        ...
        const embed = new Discord.RichEmbed()
            ...
            .addField(video.channelTitle, `[${video.title.replace(/\\/g, '\\\\').replace(/`/g, '\`').replace(/\*/g, '\\*').replace(/_/g, '\\_').replace(/~/g, '\\~')}](${video.url})`, musicbot.inlineEmbeds)
        ...
        if (queue.songs.length > 11) {
          ...
          newSongs.forEach(s => {
            const i = s.map((video) => (
                `**${video.position + 1}:** __${video.title.replace(/\\/g, '\\\\').replace(/`/g, '\`').replace(/\*/g, '\\*').replace(/_/g, '\\_').replace(/~/g, '\\~')}__`
            )).join('\n\n');
          ...
          let newSongs = musicbot.queues.get(msg.guild.id).songs.map((video) => (`**${video.position + 1}:** __${video.title.replace(/\\/g, '\\\\').replace(/`/g, '\`').replace(/\*/g, '\\*').replace(/_/g, '\\_').replace(/~/g, '\\~')}__`)).join('\n\n');
          const embed = new Discord.RichEmbed();
          embed.setAuthor('Queued Songs', client.user.avatarURL);
          embed.setColor(musicbot.embedColor);
            ...
            musicbot.searcher.search(searchstring, {
              type: 'video'
                ...
                .then(searchResult => {
                  ...
                  const startTheFun = async (videos, max) => {
                    if (msg.channel.permissionsFor(msg.guild.me).has('EMBED_LINKS')) {
                              ...
                              errors: ['time']
                                ...
                                .then(collected => {
                                  ...
                                  if (song_number >= 0) {
                                    ...
                                    embed.setAuthor('Adding To Queue', client.user.avatarURL);
                                    let songTitle = videos[song_number].title.replace(/\\/g, '\\\\')
                                        .replace(/`/g, '\`')
                                        .replace(/\*/g, '\\*')
                                        .replace(/_/g, '\\_')
                                        .replace(/~/g, '\\~');
                      ...
                      const vids = videos.map((video, index) => (
                          `**${index + 1}:** __${video.title.replace(/\\/g, '\\\\').replace(/`/g, '\`').replace(/\*/g, '\\*').replace(/_/g, '\\_').replace(/~/g, '\\~')}__`
                      )).join('\n\n');
                  ...
                  let videos = [];
                  for (let i = 0; i < 99; i++) {
                    let result = searchResult.currentPage[i];
    ...
    musicbot.executeQueue = (msg, queue) => {
      ...
      new Promise((resolve, reject) => {
        ...
        if (musicbot.messageNewSong === true && queue.last && musicbot.queues.get(msg.guild.id).loop !== "song") {
          ...
          if (msg.channel.permissionsFor(msg.guild.me).has('EMBED_LINKS')) {
            const embed = new Discord.RichEmbed()
                .setAuthor("Now Playing", `${req !== null ? req.displayAvatarURL : null}`)
                .setThumbnail(`https://img.youtube.com/vi/${video.id}/maxresdefault.jpg`)
                .setDescription(`[${video.title.replace(/\\/g, '\\\\').replace(/`/g, '\`').replace(/\*/g, '\\*').replace(/_/g, '\\_').replace(/~/g, '\\~')}](${video.url}) by [${video.channelTitle}](${video.channelURL})`)
                .setColor(musicbot.embedColor)
